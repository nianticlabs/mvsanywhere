...
def process_depth(
    ...
    pts = (
        ref_P_inv[:3, :3] @ (ref_K_inv @ (pts * ref_depth.unsqueeze(0)).view(3, n_pts))
        + ref_P_inv[:3, 3, None]
    )
    ...
    for b in range(n_batches):
        ...
        pts_reproj = (
            torch.bmm(src_Ps_batch[:, :3, :3], pts.unsqueeze(0).repeat(n_batch_imgs, 1, 1))
            + src_Ps_batch[:, :3, 3, None]
        )
        ...
        z_sample = F.grid_sample(
            src_depths_batch.unsqueeze(1),
            grid,
            mode="nearest",
            align_corners=True,
            padding_mode="zeros",
        ...
        pts_sample = torch.bmm(
            src_Ps_batch[:, :3, :3].transpose(2, 1), pts_sample - src_Ps_batch[:, :3, 3, None]
        )
    ...
    for i in range(n_src_imgs):
        ...
        invalid_idx = torch.isnan(
            pts_sample_i
        )  # filter out NaNs from div/0 due to grid sample zero padding
