...
class SevenScenesDataset(GenericMVSDataset):
    ...
    def get_valid_frame_ids(self, split, scan, store_computed=True):
        ...
        if os.path.exists(valid_frame_path):
            ...
            all_frame_ids = [
                x.strip("frame-").strip(".pose.txt") for x in all_frame_ids if ".pose.txt" in x
            ]
            ...
            for frame_id in all_frame_ids:
                color_filepath = os.path.join(scan_dir, f"frame-{frame_id}.color.png")
                if not os.path.isfile(color_filepath):
                    dist_to_last_valid_frame += 1
                    bad_file_count += 1
                    continue

                depth_filepath = os.path.join(scan_dir, f"frame-{frame_id}.depth.png")
                if not os.path.isfile(depth_filepath):
                    dist_to_last_valid_frame += 1
                    bad_file_count += 1
                    continue

                pose_filepath = os.path.join(scan_dir, f"frame-{frame_id}.pose.txt")
    ...
    def get_color_filepath(self, scan_id, frame_id):
        ...
        cached_resized_path = os.path.join(
            scene_path, f"frame-{frame_id}.color.{self.image_width}.png"
        )
        ...
        return os.path.join(scene_path, f"frame-{frame_id}.color.png")
    ...
    def get_high_res_color_filepath(self, scan_id, frame_id):
        ...
        cached_resized_path = os.path.join(
            scene_path, f"frame-{frame_id}.color.{self.high_res_image_height}.png"
        )
        # check if we have cached resized images on disk first
        if os.path.exists(cached_resized_path):
            return cached_resized_path
    ...
    def get_cached_depth_filepath(self, scan_id, frame_id):
        ...
        cached_resized_path = os.path.join(
            scene_path, f"frame-{frame_id}.depth.proj.{self.depth_width}.png"
        )
    ...
    def get_full_res_depth_filepath(self, scan_id, frame_id):
        ...
        return os.path.join(scene_path, f"frame-{frame_id}.depth.proj.png")
