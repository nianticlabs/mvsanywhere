...
import doubletake.modules.cost_volume as cost_volume
import doubletake.tools.torch_point_cloud_fusion as torch_point_cloud_fusion
from doubletake.experiment_modules.depth_model import DepthModel
from doubletake.utils.dataset_utils import get_dataset
from doubletake.utils.generic_utils import reverse_imagenet_normalize, to_gpu
...
def main(opts):
    ...
    # output location
    pc_output_folder_name = (
        f"{N_CONSISTENT_THRESH}_{Z_THRESH}_{VOXEL_DOWNSAMPLE}_{opts.fusion_max_depth}"
    )
    ...
    with torch.inference_mode():
        for scan in tqdm(scans):
            ...
            pcd_pred.colors = o3d.utility.Vector3dVector(fused_rgb / 255.0)
