...
class NerfStudioDataset(GenericMVSDataset):
    """
    Dataset class for NerfStudio data - i.e. data which is ready for splatting
    ...
    def _get_frame_ids(self, split, scan):
        ...
        with open(self.dataset_path / scan / "transforms.json") as f:
            data = json.load(f)
    ...
    def load_pose(self, scan_id, frame_id):
        ...
        world_T_cam = torch.tensor(frame_metadata["transform_matrix"], dtype=torch.float32).view(4, 4)
    ...
    def load_capture_metadata(self, scan_id):
        """Reads a nerfstudio transforms file and loads metadata for that scan into
        self.capture_metadata
    ...
    def load_target_size_depth_and_mask(self, scan_id, frame_id, crop=None):
        ...
        """
        return self.load_full_res_depth_and_mask(scan_id, frame_id, crop)
