...
class CostVolumeManager(nn.Module):
    ...
    def warp_features(
        ...
        Args:
            ...
            depth_plane_b1hw: depth plane to use for every spatial location.
    ...
    def to_fast(self) -> "FastCostVolumeManager":
        manager = FastCostVolumeManager(
            self.matching_height,
            self.matching_width,
...
class FastCostVolumeManager(CostVolumeManager):
    ...
    def build_cost_volume(
        ...
        # Sum over the frames
        dot_product_bkhw = dot_product_brkhw.mean(dim=1)
