...
class FeatureMeshHintVolumeManager(CostVolumeManager):
    ...
    def to_fast(self) -> "FastFeatureMeshHintVolumeManager":
        manager = FastFeatureMeshHintVolumeManager(
            self.matching_height,
            self.matching_width,
            num_depth_bins=self.num_depth_bins,
        )
        manager.mlp = self.mlp
        manager.hint_mlp = self.hint_mlp
        return manager


class FastFeatureMeshHintVolumeManager(FeatureMeshHintVolumeManager):
    """
    Class to build a feature volume from extracted features of an input
    reference image and N source images.
    ...
    def build_cost_volume(
        ...
        depth_hint_sampled_weights_b1hw[~prev_mesh_hint_depth_mask_b1hw] = 0
        depth_hint_sampled_weights_bdhw = depth_hint_sampled_weights_b1hw.expand(
            hint_map_bdhw.shape
        )

        # run through the MLP!
        matching_output_Bhw1 = self.mlp(mlp_input_features_Bhwc)
        combined_output_Bhw1 = self.hint_mlp(
            torch.cat(
                [
                    matching_output_Bhw1,
                    hint_map_bdhw.flatten(0, 1).unsqueeze(-1),
                    depth_hint_sampled_weights_bdhw.flatten(0, 1).unsqueeze(-1),
                ],
                dim=-1,
            )
        )
