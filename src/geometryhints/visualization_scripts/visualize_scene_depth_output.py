...
def main(opts):
    ...
    # normals computer
    compute_normals_full = NormalGenerator(large_format_size[0], large_format_size[1]).cuda()
        ...
        for scan in tqdm(scans):
            ...
            for _, batch in enumerate(tqdm(dataloader)):
                ...
                lower_image_panel_k3hw = F.interpolate(
                    src_images_k3hw,
                    size=(src_format_size[0], src_format_size[1]),
                    mode="bilinear",
                    align_corners=False,
                )
                ...
                gt_mask_b1hw = ((gt_depth_b1hw > 1e-3) & (gt_depth_b1hw < 10)).float()
                ...
                normals_b3hw = compute_normals_full(gt_depth_b1hw, invK_full_depth_b44)
                ...
                horizontal_buffer = torch.ones(
                    gt_depth_3hw.shape[0], buffer_gap, gt_depth_3hw.shape[2]
                ).cuda()
                gt_column = [gt_depth_3hw, horizontal_buffer, gt_normals_3hw]
                gt_column_np = [
                    element.permute(1, 2, 0).cpu().detach().numpy() * 255 for element in gt_column
                ]
                ...
                our_column_np = [
                    element.permute(1, 2, 0).cpu().detach().numpy() * 255 for element in our_column
                ]
                ...
                main_color_3hw[
                    :, main_color_3hw.shape[1] - lower_image_panel_3hw.shape[1] :, :
                ] = lower_image_panel_3hw

                # append image to lowest cost
                horizontal_buffer = torch.ones(
                    main_color_3hw.shape[0], buffer_gap, main_color_3hw.shape[2]
                ).cuda()
                color_panel = torch.cat(
                    [our_lowest_cost_3hw, horizontal_buffer, main_color_3hw], dim=1
                )
                ...
                pad_height = (
                    16 - (final_image_np.shape[0] % 16) if final_image_np.shape[0] % 16 != 0 else 0
                )
                pad_width = (
                    16 - (final_image_np.shape[1] % 16) if final_image_np.shape[1] % 16 != 0 else 0
                )
