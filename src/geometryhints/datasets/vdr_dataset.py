...
class VDRDataset(GenericMVSDataset):
    ...
    def load_full_res_depth_and_mask(self, scan_id, frame_id):
        ...
        full_res_depth = torch.from_numpy(
            np.fromfile(full_res_depth_filepath, dtype=np.float32).reshape(
                -1, self.native_depth_width
            )
        ).unsqueeze(0)
        ...
        conf = torch.from_numpy(
            np.fromfile(confidence_filepath, dtype=np.uint8).reshape(-1, self.native_depth_width)
        ).unsqueeze(0)
    ...
    def load_target_size_depth_and_mask(self, scan_id, frame_id):
        ...
        if os.path.exists(depth_filepath):
            depth = torch.from_numpy(
                np.fromfile(depth_filepath, dtype=np.float32).reshape(-1, self.depth_width)
            ).unsqueeze(0)
        else:
            depth_filepath = self.get_full_res_depth_filepath(scan_id, frame_id)
            depth = torch.from_numpy(
                np.fromfile(depth_filepath, dtype=np.float32).reshape(-1, self.native_depth_width)
            ).unsqueeze(0)
        ...
        if os.path.exists(confidence_filepath):
            conf = torch.from_numpy(
                np.fromfile(confidence_filepath, dtype=np.uint8).reshape(-1, self.depth_width)
            ).unsqueeze(0)
        else:
            confidence_filepath = self.get_full_res_confidence_filepath(scan_id, frame_id)
            conf = torch.from_numpy(
                np.fromfile(confidence_filepath, dtype=np.uint8).reshape(
                    -1, self.native_depth_width
                )
            ).unsqueeze(0)
