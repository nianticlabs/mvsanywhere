...
from geometryhints.utils.generic_utils import read_image_file
...
class ARKitDataset(GenericMVSDataset):
    ...
    def get_valid_frame_ids(self, split, scan, store_computed=True):
        ...
        if os.path.exists(valid_frame_path):
            ...
            all_frame_ids = [x.strip(".png") for x in all_frame_ids if ".png" in x]
            ...
            for frame_id in all_frame_ids:
                try:
                    ...
                    # Load image
                    _ = read_image_file(
                        color_filename, height=self.image_height, width=self.image_width
                    )

                    # check pose
                    world_T_cam, _ = self.load_pose(scan, frame_id)
                    if (
                        np.isnan(np.sum(world_T_cam))
                        or np.isinf(np.sum(world_T_cam))
                        or np.isneginf(np.sum(world_T_cam))
                    ):
            ...
            print(
                f"Scene {scan} has {bad_file_count} bad frame files "
    ...
    def get_color_filepath(self, scan_id, frame_id):
        ...
        cached_resized_path = os.path.join(color_path, f"{frame_id}_{self.image_width}.png")
    ...
    def get_high_res_color_filepath(self, scan_id, frame_id):
        ...
        cached_resized_path = os.path.join(
            color_path, f"{frame_id}_{self.high_res_image_height}.png"
        )
        # check if we have cached resized images on disk first
        if os.path.exists(cached_resized_path):
            return cached_resized_path
    ...
    def get_pose_filepath(self, scan_id, frame_id):
        ...
        """
        return os.path.join(self.scenes_path, scan_id, "poses", f"{frame_id}.txt")
...
def path_parser(root_path, data_source="TagBA"):
    ...
    if data_source == "TagBA":
        path_dict["camera_pose"] = os.path.join(full_path, "TagBA", "CameraTrajectory-BA.txt")
        path_dict["cam_intrinsic"] = os.path.join(full_path, "camera_intrinsics.json")
        ...
        path_dict["cam_intrinsic"] = os.path.join(full_path, "device_parameter.txt")
    return path_dict
...
def load_camera_pose(cam_pose_dir, use_homogenous=True, data_source="TagBA"):
    if cam_pose_dir is not None and os.path.isfile(cam_pose_dir):
        ...
        raise FileNotFoundError("Given camera pose dir:{} not found".format(cam_pose_dir))
    ...
    def process(line_data_list):
        ...
        if data_source == "ARKit":
            rot_mat = rot_mat.dot(np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]]))
...
def load_camera_intrinsic(cam_file, data_source="TagBA"):
    ...
    if data_source == "TagBA":
        ...
        cam_dict["shape"] = (w, h)
        cam_dict["dist_coeff"] = np.array(cam_info["distortion_coefficients"], dtype=float)
    elif data_source == "Open3D":
        with open(cam_file, "r") as f:
            cam_info = json.load(f)
        cam_dict["K"] = (
            np.array(cam_info["intrinsic_matrix"], dtype=float).reshape(3, 3).transpose()
        )
        ...
        raise NotImplementedError("Data parsing for source: {} not implemented".format(data_source))
