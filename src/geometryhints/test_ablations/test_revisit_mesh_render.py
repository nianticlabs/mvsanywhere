    ...
    with torch.inference_mode():
        ...
        for scan in tqdm(scans):
            ...
            for batch_ind, batch in enumerate(tqdm(dataloader, desc="Second pass")):
                ...
                outputs["depth_pred_s0_b1hw"] = torch.cat(filled_depth_maps, 0)
                # outputs["depth_pred_s0_b1hw"] = cur_data["depth_hint_b1hw"]#torch.cat(filled_depth_maps, 0)
                # for depth_pred_ind, depth_pred_1hw in enumerate(
                #     outputs["depth_pred_s0_b1hw"]
                # ):
                #     if cur_data["depth_hint_mask_b_b1hw"][depth_pred_ind].sum() == 0:
                #         outputs["depth_pred_s0_b1hw"][depth_pred_ind] = 1
