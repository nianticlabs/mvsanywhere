...
To use Ray, run:
```bash
$ python -m geometryhints.ray --config PATH/TO/CONFIG.YAML --data_config configs/data/scannet_default_train.yaml \
...
class RayRunner:
    ...
    def submit_job(self, options: options.Options) -> None:
        ...
        training_command = f"python -m geometryhints.ray.entrypoint {' '.join(training_args)}"
