...
from geometryhints.tools.mesh_renderer import (
    DEFAULT_CAM_FRUSTUM_MATERIAL,
    DEFAULT_MESH_MATERIAL,
    Renderer,
    SmoothBirdsEyeCamera,
    camera_marker,
    create_light_array,
    get_image_box,
    transform_trimesh,
...
from geometryhints.utils.generic_utils import to_gpu
...
def main(opts):
    ...
    print(f"Incremental Mesh Output directory:" f"\n{incremental_mesh_output_dir} ".center(80, "#"))
    ...
    with torch.inference_mode():
        for scan in tqdm(scans):
            ...
            Path(os.path.join(incremental_mesh_output_dir, scan)).mkdir(parents=True, exist_ok=True)
            ...
            for batch_ind, batch in enumerate(tqdm(dataloader)):
                ...
                trimesh_path = os.path.join(incremental_mesh_output_dir, scan, f"{frame_id}.ply")
                ...
                if all_meshes_precomputed and opts.use_precomputed_partial_meshes:
                    ...
                    if not opts.run_fusion:
                        raise Exception(
                            "No precomputed partial mesh found and " "run_fusion is disabled."
                        )

                    # check if depths are precomputed.
                    pickled_depths_path = os.path.join(depth_output_dir, scan, f"{frame_id}.pickle")
                    ...
                    if Path(pickled_depths_path).is_file():
                        ...
                        if opts.cache_depths:
                            ...
                            output_path = os.path.join(depth_output_dir, scan, f"{frame_id}.pickle")
                    ...
                    fuser.fuse_frames(
                        depth_pred, cur_data["K_s0_b44"], cur_data["cam_T_world_b44"], color_frame
                    )

                    Path(os.path.join(incremental_mesh_output_dir, scan)).mkdir(
                        parents=True, exist_ok=True
                    )
                    mesh_path = os.path.join(incremental_mesh_output_dir, scan, f"{frame_id}.ply")
                ...
                meshes = [] if scene_trimesh_mesh is None else [scene_trimesh_mesh]
                ...
                meshes = [] if scene_trimesh_mesh is None else [scene_trimesh_mesh]
                mesh_materials = [None] if opts.fuse_color else [DEFAULT_MESH_MATERIAL]

                fpv_camera = trimesh.scene.Camera(
                    resolution=(render_height, render_width), focal=(K_33[0][0], K_33[1][1])
                )
                ...
                our_depth_3hw = colormap_image(depth_pred.squeeze(0), vmin=0, vmax=3.0)
                our_depth_hw3 = our_depth_3hw.permute(1, 2, 0)
                pil_depth = Image.fromarray(np.uint8(our_depth_hw3.cpu().detach().numpy() * 255))
                ...
                birdeye_world_T_cam_44 = smooth_birdseye.get_bird_eye_trans(
                    scene_trimesh_mesh, fpv_pose=world_T_cam_44
                )
            ...
            save_viz_video_frames(
                mesh_render_fpv_frames,
                os.path.join(video_output_dir, scan.replace("/", "_") + "_fpv.mp4"),
                fps=fps,
            )
            save_viz_video_frames(
                mesh_render_birdeye_frames,
                os.path.join(video_output_dir, scan.replace("/", "_") + "_birdseye.mp4"),
                fps=fps,
            )
