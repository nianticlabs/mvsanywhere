...
class Renderer:
    ...
    def render_mesh_cull_composite(self, alpha, **kwargs):
        ...
        non_culled_render = self.render_mesh(
            **kwargs, render_flags=pyrender.RenderFlags.SKIP_CULL_FACES
        )
...
def render_colour(renderer, meshes, world_T_cam, K, height=256, width=320):
    colour = renderer.render_mesh(meshes, height, width, world_T_cam, K, get_colour=True)
    if colour is None:
        del renderer
        renderer = Renderer()
        colour = renderer.render_mesh(meshes, height, width, world_T_cam, K, get_colour=True)
...
class SmoothBirdsEyeCamera:
    ...
    def get_bird_eye_trans(
        ...
        if self.current_mean_loc is None:
            ...
            self.current_mean_loc = (
                self.mean_mesh_moving_alpha * self.current_mean_loc
...
def camera_marker(
    ...
    points = np.array([(0, 0, 0), (-x, -y, z), (x, -y, z), (x, y, z), (-x, y, z)], dtype=float)
    ...
    np_vertices[:, 0] = (
        np_vertices[:, 0] + np.tan(np.deg2rad(camera.fov[1]) / 2.0) * np_vertices[:, 2]
    )
    np_vertices[:, 1] = (
        np_vertices[:, 1] + np.tan(np.deg2rad(camera.fov[0]) / 2.0) * np_vertices[:, 2]
    )
    ...
    rect = trimesh.creation.box(
        [np.tan(np.deg2rad(camera.fov[1]) / 2.0) * 2.0 * cam_marker_size, rect_width, rect_width]
    )
    np_vertices = np.array(rect.vertices)
    np_vertices[:, 2] += cam_marker_size
    np_vertices[:, 1] = (
        np_vertices[:, 1] - np.tan(np.deg2rad(camera.fov[0]) / 2.0) * cam_marker_size
    )
    ...
    rect = trimesh.creation.box(
        [rect_width, np.tan(np.deg2rad(camera.fov[0]) / 2.0) * 2.0 * cam_marker_size, rect_width]
    )
    np_vertices = np.array(rect.vertices)
    np_vertices[:, 2] += cam_marker_size
    np_vertices[:, 0] = (
        np_vertices[:, 0] - np.tan(np.deg2rad(camera.fov[1]) / 2.0) * cam_marker_size
    )
...
def transform_trimesh(mesh, transform):
    ...
    np_vertices = (
        transform @ np.concatenate([np_vertices, np.ones((np_vertices.shape[0], 1))], 1).T
    ).T
