...
__global__ void ClassifyVoxelKernel(
  ...
  const uint numActive = activeIndices.size(0);
  ...
  for (uint tid = id; tid < numActive; tid += num_threads) {
    int gx = activeIndices[tid][2];
    int gy = activeIndices[tid][1];
    int gz = activeIndices[tid][0];
...
__global__ void GenerateFacesKernel(
  ...
  for (uint tid = id; tid < activeVoxels; tid += num_threads) {
    ...
    const uint H = vol.size(1) - 1;
    const uint W = vol.size(2) - 1;
    int gx = activeIndices[voxel][2];
    int gy = activeIndices[voxel][1];
    int gz = activeIndices[voxel][0];
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> MarchingCubesCuda(
  ...
  dim3 activeGrid((numActive + threads - 1) / threads, 1, 1);
  if (grid.x > 65535) {
    grid.x = 65535;
  }

  auto d_voxelVerts =
      at::zeros({numActive}, at::TensorOptions().dtype(at::kInt))
          .to(vol.device());
  ...
  // two arrays - d_voxelOccupied and d_voxelVertices to global memory,ß
  // which stores the occupancy state and number of voxel vertices per voxel.
  ClassifyVoxelKernel<<<activeGrid, threads, 0, stream>>>(
  ...
  int lastElement = d_voxelVerts[numActive - 1].cpu().item<int>();
  int lastScan = d_voxelOccupiedScan[numActive - 1].cpu().item<int>();
  ...
  CompactVoxelsKernel<<<activeGrid, threads, 0, stream>>>(
  ...
  auto d_voxelVertsScan = at::zeros({numActive}, opt);
  ThrustScanWrapper(
  ...
  lastElement = d_voxelVerts[numActive - 1].cpu().item<int>();
  lastScan = d_voxelVertsScan[numActive - 1].cpu().item<int>();
