...
def print_table(mask_type="tf", scores_table="online"):
    ...
    if scores_table  == "offline":
        ...
        if mask_type == "tf":
            all_scores_dict["COLMAP~\cite{schonberger2016pixelwise,schoenberger2016sfm} & No"] = {metric: value for metric, value in zip(metrics, [10.22/100,11.88/100,11.05/100,0.509,0.474,0.489])}
            all_scores_dict["ATLAS~\cite{murez2020atlas} & Yes"] = {metric: value for metric, value in zip(metrics, [7.16/100,7.61/100,7.38/100,0.675,0.605,0.636])}
            all_scores_dict["3DVNet~\cite{rich20213dvnet} & Yes"] = {metric: value for metric, value in zip(metrics, [6.73/100,7.72/100,7.22/100,0.655,0.596,0.621])}

        if mask_type == "tf":
            all_scores_dict["TransformerFusion~\cite{bozic2021transformerfusion} & Yes"] = {metric: value for metric, value in zip(metrics, [5.5194/100, 8.2680/100, 6.8937/100, 0.7285, 0.5999, 0.6554])}
        else:
            all_scores_dict["TransformerFusion~\cite{bozic2021transformerfusion} & Yes"] = {metric: value for metric, value in zip(metrics, [4.6822/100, 8.2680/100, 6.4751/100, 0.6984, 0.5999, 0.6442])}
            
        if mask_type == "tf":
            all_scores_dict["VoRTX~\cite{stier2021vortx} & Yes"] = {metric: value for metric, value in zip(metrics, [4.3138/100, 7.2266/100, 5.7702/100, 0.7675, 0.6508, 0.7030])}
        else:
            all_scores_dict["VoRTX~\cite{stier2021vortx} & Yes"] = {metric: value for metric, value in zip(metrics, [4.3794/100, 7.2266/100, 5.8030/100, 0.7257, 0.6508, 0.6851])}


        all_scores_dict["SimpleRecon~\cite{sayed2022simplerecon} (offline) (2cm)  & No"] = json.load(open(f"/mnt/nas3/personal/mohameds/geometry_hints/outputs/sr_new_scores_offline/scannet/default/meshes/0.02_3.5_ours_neg_trunc/{scores_json_name}"))["overall"]

        if mask_type == "tf":
            all_scores_dict["FineRecon~\cite{Stier_2023_ICCV} (1cm) & Yes"] = {metric: value for metric, value in zip(metrics, [5.2537/100, 5.0599/100, 5.1568/100, 0.7794, 0.7373, 0.7560])}
        else:
            all_scores_dict["FineRecon~\cite{Stier_2023_ICCV} (1cm) & Yes"] = {metric: value for metric, value in zip(metrics, [4.9243/100, 5.0599/100, 4.9921/100, 0.6869, 0.7373, 0.7098])}
            

        all_scores_dict["\\textbf{Ours} (two-pass) (2cm)& No"] = json.load(open(f"/mnt/nas3/personal/mohameds/geometry_hints/outputs/final_model_new_renders_trunc_trick/scannet/default/meshes/0.02_3.5_ours/{scores_json_name}"))["overall"]
