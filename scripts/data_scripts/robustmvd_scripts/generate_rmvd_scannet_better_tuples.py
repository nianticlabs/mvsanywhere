...
def offline_dense_dvmvs_tuples(scan, poses, n_measurement_frames, valid_frames):
    """
    ...
    rmvd_tuples = np.loadtxt('data_splits/ScanNetv2/rmvd_split/test_eight_view_rmvd.txt', dtype=object)
    valid_ref_frames = rmvd_tuples[rmvd_tuples[:, 0] == scan][:, 1]

    for i in range(0, len(poses)):

        if valid_frames[i].split()[1] not in valid_ref_frames:
            continue
...
def crawl_subprocess_long(opts_temp_filepath, scan, count, progress):
    ...
    if opts.datasets[0].frame_tuple_type == "default":
        ...
        samples = offline_dense_dvmvs_tuples(scan, poses, n_measurement_frames, valid_frames)
    else:
