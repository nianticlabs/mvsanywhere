...
def crawl_subprocess_long(opts_temp_filepath, scan, count, progress):
    ...
    for frame_ind, frame_line in enumerate(valid_frames):
        frame_ind_to_frame_id[frame_ind] = frame_line.strip().split(" ")[1]
        if len(frame_line.strip().split(" ")) > 3:
            is_high_res.append(frame_line.strip().split(" ")[3] == "1")
            if frame_line.strip().split(" ")[3] == "1":
                high_res_count += 1
        else:
            is_high_res.append(True)
    ...
    # # is_high_res = []
    # for frame_ind, frame_line in enumerate(valid_frames):
    #     # if len(frame_line.strip().split(" ")) > 3 and frame_line.strip().split(" ")[3] == 1:
    #     frame_ind_to_frame_id[frame_ind] = frame_line.strip().split(" ")[1]
    ...
    # print(high_res_count, len(poses), len(valid_frames))
    ...
    calculated_step = opts.dvmvs_train_crawl_step
    samples = []
    for offset, multiplier, is_backward in [
        ...
        for i in range(start, limit, step):
            ...
            # check if this frame has high res depth, if not don't use it as a current frame
            if not is_high_res[i]:
                continue

            # start building tuple
                ...
                if not reached_sequence_limit:
                    ...
                    check3 = is_valid_pair(
                        ...
                        (
                            ...
                            * opts.train_max_pose_distance_mult
                        ),
                        t_norm_threshold=(
